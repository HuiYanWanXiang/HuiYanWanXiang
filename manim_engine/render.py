...
BASE_URL = os.environ.get("OPENAI_BASE_URL", "https://api.apishop.qzz.io/v1")
...
def extract_llm_text(resp) -> str:
    """
    兼容标准 OpenAI 响应、dict，以及 sub2api 可能直接回传的字符串。
    """
    if resp is None:
        return ""
...
def post_chat_completions(messages, temperature: float, max_tokens: int):
...
def main():
    ...
    print(
        f"[RENDER] llm_config | base_url={BASE_URL} | model={MODEL} | api_key={mask_api_key(os.environ['OPENAI_API_KEY'])}"
    )
