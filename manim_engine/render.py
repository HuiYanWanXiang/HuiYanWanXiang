...
def quality_check(code: str, original_request: str) -> None:
    """
    轻量质量检查：不追求“完美”，只拦截明显垃圾输出。
    """

    req = original_request.lower()

    # (A) 如果用户没要求画函数/坐标系，禁止 Axes/NumberPlane 等（防止莫名其妙抛物线）
    ...
    # (B) 强制 Tex/MathTex 使用 raw string，避免 \t \r \n 被 Python 吃掉 → extmass / extvelocity
    #     这里用负向前瞻：如果 Tex( " 或 Tex(' 但不是 Tex(r" / Tex(r'
    ...
    # (C) 推导类内容：至少要有多步公式（MathTex >= 2）并出现基本数学符号（= 或 \\int 或 \\frac）
    ...
    # (D) 针对“动能/功-能定理”这类关键词，要求出现 K 与 1/2（避免只写一句 K=... 就糊弄）
    if ("kinetic" in req) or ("动能" in original_request) or ("work-energy" in req) or ("功" in original_request and "能" in original_request):
...
def call_llm_raw(client: OpenAI, input_text: str) -> str:
    ...
    return strip_code_fences(resp.output_text or "")
...
def generate_code_with_retries(
    ...
    """
    生成代码，如果失败（安全/质量），就把失败原因+旧代码发回去让 LLM 重写。
    """
...
def find_latest_mp4(media_dir: Path) -> Path | None:
    """在本次 run 的 media 目录下找最新 mp4"""
...
def main():
    ...
    # 目标时长：软约束写入提示词，让 LLM 调整 run_time / wait
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise SystemExit(
            ...
            "CMD temp:         set OPENAI_API_KEY=sk-...\n"
            "PowerShell temp:  $env:OPENAI_API_KEY='sk-...'\n"
            "If you used setx, close ALL terminals/VSCode and reopen."
        )
    ...
    # 给 LLM 的输入（注意：这里不包含“坐标轴关键词”，避免影响 quality_check 的判断）
    initial_user_prompt = (
    ...
    # -------- 2) 渲染 + 自动修复循环（渲染失败就把 stderr 发回去让 LLM 修）--------
