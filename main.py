...
HTML_MAX_TOKENS = int(os.getenv("HTML_MAX_TOKENS", "1024").strip())
...
def _extract_llm_text(resp: Any) -> str:
    """
    兼容标准 OpenAI 响应、dict，以及 sub2api 等中转层可能直接返回的字符串。
    """
    if resp is None:
        return ""
...
async def _post_chat_completions_async(
    *,
    ...
    if resp.status_code >= 400:
        raise RuntimeError(resp.text[:3000])

    content_type = (resp.headers.get("content-type") or "").lower()
    if "application/json" in content_type:
        return resp.json()
    if resp.text.lstrip().startswith("<!doctype html") or "<html" in resp.text[:200].lower():
        raise RuntimeError(resp.text[:3000])
    return resp.text


async def _post_chat_completions_streaming_async(
    *,
    api_key: str,
    base_url: str,
    model: str,
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        async with client.stream("POST", url, headers=headers, json=payload) as resp:
            if resp.status_code >= 400:
                ...
                raise RuntimeError(body.decode("utf-8", errors="replace")[:3000])
            ...
            if "text/event-stream" not in content_type:
                ...
                if "application/json" in content_type:
                    data = httpx.Response(200, content=body, headers=resp.headers).json()
...
async def _html_worker_streaming(job_id: str, prompt: str, username: str, api_key: str, base_url: str, model: str) -> None:
    job_prompt = prompt
    logger.info(f"[HTML_JOB:{job_id}] start | prompt='{prompt}' | model={model} | username={username}")
    logger.info(
        f"[HTML_JOB:{job_id}] llm_config | base_url={base_url} | model={model} | api_key={_mask_api_key(api_key)}"
    )

    await _set_job(job_id, {"status": "running"})

    try:
        final_system_prompt = _build_system_prompt(job_prompt)
        if "知识" in final_system_prompt:
        ...
        user_prompt = (
            ...
            "请生成一个简洁的中文教学网页。"
            "控制篇幅，不要长篇大论。"
            "页面结构尽量精炼，模块数量适中，文案偏短。"
            "直接返回完整 HTML，不要输出 Markdown 代码块。"
        )
        messages = [
        ...
        logger.info(f"[HTML_JOB:{job_id}] calling LLM stream... | max_tokens={HTML_MAX_TOKENS}")
        raw_content = await _post_chat_completions_streaming_async(
            api_key=api_key,
            base_url=base_url,
            model=model,
            messages=messages,
        ...
        ts = _now_ts()
        safe_prompt = re.sub(r'[\\/*?:"<>|]', "", job_prompt)[:15]
        ...
        record_error("html", str(e), f"job_id={job_id} | prompt={job_prompt} | model={model} | base_url={base_url}")

    except Exception as e:
        msg = str(e)
        await _set_job(job_id, {"status": "error", "error": msg})
        logger.error(f"[HTML_JOB:{job_id}] Exception: {msg}")
        record_error("html", msg, f"job_id={job_id} | prompt={job_prompt} | model={model} | base_url={base_url}")
...
async def _html_worker(job_id: str, prompt: str, username: str, api_key: str, base_url: str, model: str) -> None:
    ...
    try:
        ...
        logger.info(f"[HTML_JOB:{job_id}] calling LLM... | max_tokens={HTML_MAX_TOKENS}")
        resp = await _post_chat_completions_async(
            api_key=api_key,
            base_url=base_url,
            model=model,
            messages=[
        ...
        raw_content = _extract_llm_text(resp)
        clean_html = raw_content.replace("```html", "").replace("```", "").strip()
        if not clean_html.endswith("</html>"):
            logger.warning(f"[HTML_JOB:{job_id}] html truncated, auto append </html>")
            clean_html += "\n\n</body></html>"
...
async def generate_html(
    ...
    asyncio.create_task(_html_worker_streaming(
        job_id=job_id,
        prompt=user_prompt,
        username=current_user.username,
