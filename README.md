...
易演课堂面向物理、数学等教学场景，围绕“**自然语言输入 -> 教学内容自动生成**”构建。  
当前版本已经实现两条核心内容生产链路：
...
当前版本会在启动时自动读取项目根目录下的 `.env` 文件，所以只要把配置写进 `.env`，`main.py`、`manim_engine/router.py` 和 `debug_net.py` 都会直接使用。

建议在项目根目录放置如下内容：

```env
VIDEO_API_KEY=你的API_KEY
VIDEO_BASE_URL=https://api.apishop.qzz.io/v1
VIDEO_MODEL=gpt-5.4
HTML_MAX_TOKENS=1024
```

说明：
- `VIDEO_API_KEY` 会同时被 `main.py`、`manim_engine/router.py` 和 `debug_net.py` 使用
- `VIDEO_BASE_URL` 用于兼容 OpenAI 协议的模型服务地址
- `VIDEO_MODEL` 用于 HTML 生成和视频生成时的默认模型名
- `HTML_MAX_TOKENS` 控制 HTML 生成接口的默认输出长度

如果你使用的是不同的模型网关，只需要修改这几个变量即可，不需要再改代码里的硬编码值。

如果你是第一次部署，直接复制一份 `.env` 并填入自己的 `VIDEO_API_KEY` 即可，不需要手动在终端里额外导出环境变量。
...
易演课堂不仅是一个“大模型生成网页 / 视频”的工具，更是一个可本地落地、可长期沉淀、可逐步扩展为教学平台的内容生成系统。
